module MeetingsHelper
  ...
  def meeting_advisor_subjects(type)
    case type
      when /Career/   then hf_career_advisor_discussed_for_select
      when /Academic/ then hf_academic_advisor_discussed_for_select
      when 'Wellness' then hf_wellness_advisor_discussed_for_select
      when 'DEIB'     then hf_diversity_navigator_for_select
      when 'Assist Dean' then hf_assist_dean_for_select
      else hf_academic_advisor_discussed_for_select
    end
  end


  def meeting_outcomes_for(type)
    # Similar case statement for outcomes...
    case type
      when /Academic/ then hf_meeting_ipas_for_select
      when /Career/   then hf_meeting_ipps_for_select
      when 'Wellness' then hf_wellness_advisor_discussed_for_select
      when 'DEIB'     then ""
