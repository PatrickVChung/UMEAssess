class WbaGraphsController < ApplicationController
  ...
  def index
    if params[:category_id].present?
      @categories, @data_series, @selected_category, @total_wbas, @max_value = hf_series_data(params[:category_id])
    end
  ...
  def wba_report
    ...
    if params[:permission_group_id].present?
      ...
      @epas_count = PermissionGroup.where(id: params[:permission_group_id]).joins(users: :epas).group('epas.epa').count
      @permission_group_id = params[:permission_group_id]
      @cohort_count = PermissionGroup.where(id: params[:permission_group_id]).joins(:users).count
      @epas_count = hf_special_sort(@epas_count)
      @epas_average = @epas_count.map{|epa, val| {epa => val/@cohort_count}}
      @users = PermissionGroup.find_by(id: params[:permission_group_id]).users.select(:id, :full_name, :sid, :email).order(:full_name)
