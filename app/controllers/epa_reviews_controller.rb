class EpaReviewsController < ApplicationController
  before_action :find_reviewable, :load_reasons #, :load_eg_members
  before_action :set_epa_review, only: [:edit, :update]
  ...
  def new
    ...
    if !Current.user.admin_or_higher? then
      @eg_members = [Current.user.full_name]
    end
    ...
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: turbo_stream.update(
          ...
          partial: "shared/modal",
          locals: {
            modal_id: "epa_edit_modal", # Pass an ID
            create_tabs: true,
            title: "Edit Review: #{@epa_review_epa} => #{hf_epa_desc_with_color(@epa_review_epa, @new_competency)}",
            body: render_to_string(partial: "epa_reviews/form", locals: { epa_review: @epa_review }),
            size: "modal-lg",
  ...
  def update
    ...
    if @epa_review.update(epa_review_params)
      ...
      respond_to do |format|
        ...
        format.html { redirect_to epa_masters_path, status: :see_other, notice: "Review updated!" }
      end
    else
      respond_to do |format|
  ...
  def get_evidence (user_id, epa_count)
    ...
    if Current.user.spec_program == 'AccessAI'
      get_ai_data
    end
  ...
  private
    ...
    def prepare_form_data
        @decision_option = ["Grounded", "Presumptive", "Distrust", "Questioned Trust", "No Decision"]
        @decision_option2 = @decision_option # Reuse the same array
        @epa_review_epa = @epa_review.epa
        @user_id = EpaMaster.find(@epa_review.reviewable_id).user_id

        # Logic for old vs new EPAs
        @epa_count = EpaMaster.where(user_id: @user_id, epa: "EPA1A").any? ? 12 : 13
        @new_competency = User.find(@user_id).new_competency

        get_evidence(@user_id, @epa_count)

        # Building the evidence strings
        str_complete = "QA Completion: #{@percent_complete[@epa_review_epa.downcase]}\r" \
                       "Total No of WBA: #{@wba[@epa_review_epa].sum}\r".html_safe
        str_wba = hf_wba_str(@wba[@epa_review_epa])
     ...
     def get_ai_data
       ...
       if File.exist?(file_name) && Current.user.spec_program == 'AccessAI'
