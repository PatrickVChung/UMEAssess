class EventsController < ApplicationController
  ...
  def create_batch_appointments
    ...
    if params[:fpStartDate1].present?
      ...
      @appointments = Event.enumerate_hours2(
        params[:fpStartDate1],
        params[:fpEndDate1],
        @time_slot,
        @advisor_type,
      ...
      if params[:advisor_type].present?
          respond_to do |format|
            ...
            format.turbo_stream do
              if params[:fpStartDate1].blank?
                    render turbo_stream: turbo_stream.replace("advisor_selection_frame", partial: "events/advisor_select")
              else
                render turbo_stream: turbo_stream.replace("detail_appointments",
                   partial: "events/detail_appointments",
                   locals: { appointments: @appointments, advisor_name: @advisor })
              end
            end

            # This handles the fallback or the standard frame request
            format.html do
              render partial: "events/detail_appointments",
                     locals: { appointments: @appointments, advisor_name: @advisor }
            end
          end
        else
          render :create_batch_appointments
        end

    else
      # This handles the initial GET request when you first navigate to the page
      render :create_batch_appointments
    end
  end
  ...
  def save_batch_appointments
      ActiveRecord::Base.transaction do
        # Params[:appointment_data] will be an array like:
        # ["04/02/2026 09:00AM|04/02/2026 09:45AM|123|Step1", "..."]
          appointments_to_save = params[:appointment_data]
        if appointments_to_save.present?
          appointments_to_save.each do |data_string|
            start_str, end_str, advisor_id, advisor_type, description = data_string.split('|')
            Event.create!(
              start_date: DateTime.strptime(start_str, "%m/%d/%Y %I:%M%p"),
              end_date: DateTime.strptime(end_str, "%m/%d/%Y %I:%M%p"),
          ...
          flash[:notice] = "Successfully saved #{appointments_to_save.count} appointments."
        else
          flash[:alert] = "No appointments were selected."
        end

    rescue ActiveRecord::RecordInvalid => e
      flash[:alert] = "Error saving batch: #{e.message}"
    end

    respond_to do |format|
        format.turbo_stream do
          render turbo_stream: [
            # Use update instead of prepend so old messages disappear
            turbo_stream.update("flash-container", partial: "layouts/flashes"),
            turbo_stream.replace("detail_appointments", partial: "events/detail_appointments")
          ]
        end
        format.html { redirect_to create_batch_appointments_events_path }
      end
  end
