class CompetenciesController < ApplicationController
  ...
  def index
    ...
    if current_user.coaching_type == "student"
      ...
      if params[:uuid].present? and  !(@comp = User.find_by(uuid: params[:uuid]).competencies.order(start_date: :desc)).empty?
        @selected_user =  User.find_by(uuid: params[:uuid])
  ...
  def load_competencies(permission_group_id, full_name)
    ...
    #@comp_remap_class_mean = hf_remap_comp(@comp_class_mean)
     ...
     @student_comp_data, @class_mean_data, @categories = hf_create_chart('Competency', @comp_data_clinical, @comp_class_mean, full_name)
     @student_comp_data_new, @class_mean_data_new, @categories_new = hf_create_chart('New Competency', @comp_remap_data_clinical, @comp_remap_class_mean, full_name)

    @epa_student ||= hf_epa2(@comp_data_clinical)
    @epa_class_mean ||= hf_epa2(@comp_class_mean)
    @epa_student_data, @epa_class_mean_data, @epa_categories = hf_create_chart('EPA', @epa_student, @epa_class_mean, full_name)
